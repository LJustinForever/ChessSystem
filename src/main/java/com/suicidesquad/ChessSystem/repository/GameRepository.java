...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    @Query("SELECT g FROM Game g WHERE (g.black = :user OR g.white = :user) AND g.state = :state ORDER BY g.id DESC")
    List<Game> findAllByUsersAndState(@Param("user") Guest user, @Param("state") Game_state state);
